...
document.getElementById("shuffler").addEventListener("click", function() {
	...
	let minSiteX =w,minSiteY=h,maxSiteX=0, maxSiteY=0;
	
	let goahead=true;

	if (allCells.length>500){
		goahead=false;
		alert('Used cells should be less than 500 to activate shuffle command.')
	}else if (fairSiteLinework.length==0){
		goahead=false;
		alert('Please draw the site/plot boundry.')
	}else if (allCells.length<=500){
		goahead=true;
	}
	...
	if(goahead){
		if(allCells.length>0){
			...
			for(let i=0; i<fairSiteLinework.length; i++){
				let vSiteT1 = new Vector(fairSiteLinework[i].v1.getX(), fairSiteLinework[i].v1.getY());
				// let vSiteT2 = new Vector(fairSiteLinework[i].v2.getX(), fairSiteLinework[i].v2.getY());
				if (vSiteT1.getX()>maxSiteX){
					maxSiteX = vSiteT1.getX();
				}
				if (vSiteT1.getY()>maxSiteY){
					maxSiteY = vSiteT1.getY();
				}
				if (vSiteT1.getX()<minSiteX){
					minSiteX = vSiteT1.getX();
				}
				if (vSiteT1.getY()<minSiteY){
					minSiteY = vSiteT1.getY();
				}
				
				// if (vSiteT2.getX()>maxX){
				// 	maxSiteX = vSiteT2.getX();
				// }
				// if (vSiteT2.getY()>maxY){
				// 	maxSiteY = vSiteT2.getY();
				// }
				// if (vSiteT2.getX()<minX){
				// 	minSiteX = vSiteT2.getX();
				// }
				// if (vSiteT2.getY()<minY){
				// 	minSiteY = vSiteT2.getY();
				// }
			}
